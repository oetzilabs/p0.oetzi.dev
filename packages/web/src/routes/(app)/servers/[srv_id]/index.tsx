import { findById, getVirtualMachineConfigList } from "@/api/servers";
...
import { ServerInfo } from "@p0/core/src/db/schema";
...
export const route = {
  preload: async (props) => {
    ...
    const vmConfigNameList = await getVirtualMachineConfigList();
      ...
      <Show when={server()} fallback={<NotFound />}>
        {(s) => (
          <div class="flex w-full flex-col h-content grow gap-2 ">
            ...
            <div class="flex flex-col gap-2 p-4 border-b w-full">
              ...
              <div class="flex flex-col gap-2 w-full">
                <div class="grid w-full grid-cols-4">
                  <For
                    each={s().vms.filter((vm) => vm.type === "jailed_vm")}
                    fallback={
